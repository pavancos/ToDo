...
void addtodo()
{
    ...
    // Convert task to lowercase and remove spaces
    string processedTask = toLower(task);
    processedTask.erase(remove_if(processedTask.begin(), processedTask.end(), ::isspace), processedTask.end());
    ...
    while (!temp.empty())
    {
        // Convert existing task name to lowercase and remove spaces
        string existingTask = temp.top().task;
        string processedExistingTask = toLower(existingTask);
        processedExistingTask.erase(remove_if(processedExistingTask.begin(), processedExistingTask.end(), ::isspace), processedExistingTask.end());

        // Check if task name and date are the same
        if (processedExistingTask == processedTask &&
            Ded.tm_mday == temp.top().dedLine.tm_mday &&
            Ded.tm_mon == temp.top().dedLine.tm_mon &&
            Ded.tm_year == temp.top().dedLine.tm_year)
        {
            cout << "\n\tTask already added!\n";
            return;
        }
